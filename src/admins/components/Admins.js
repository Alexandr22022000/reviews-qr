...
class Admins extends React.Component { //FIXME add invitations display, add emails display and check (already exist), update on every open
    render () {
        ...
        if (this.props.processingStatus === -1)
            return (
                <Dialog open={true} onClose={() => this.props.setAdminsProcessing(0)} aria-labelledby="form-dialog-title">
                    <DialogTitle>{`Invitation sent to ${this.state.email}`}</DialogTitle>
                    <DialogActions>
                        <Button onClick={() => this.props.setAdminsProcessing(0)}>
                            Ok
                        </Button>
                    </DialogActions>
                </Dialog>
        ...
        if (this.state.idForRejection)
            return (
                <Dialog open={true} onClose={() => this.setState({idForRejection: null})} aria-labelledby="form-dialog-title">
                    <DialogTitle id="form-dialog-title">{`Are you sure you want to reject admin ${this.state.nameForRejection}`}</DialogTitle>
                    <DialogActions>
                        <Button onClick={() => this.setState({idForRejection: null})}>
                            Cancel
                        </Button>
        ...
        if (this.props.object) {
            ...
            admins = admins.map(admin => (
                <ListItem>
                    ...
                    <ListItemText
                        primary={admin.name}
                        secondary={admin.isCreator ? "Owner" : "Admin"}
                    />


                    <ListItemSecondaryAction>
                        {admin.isCreator ? "" : (
                            <IconButton onClick={() => this.setState({idForRejection: admin.id, nameForRejection: admin.name})}>
                                <HighlightOffIcon />
                            </IconButton>
                        )}
                    </ListItemSecondaryAction>
            ...
            <Dialog open={this.props.open} onClose={this.onClose.bind(this)} aria-labelledby="form-dialog-title">
                ...
                <DialogContent>
                    ...
                    {!this.state.openInviteButton ? (
                        <Button fullWidth onClick={() => this.setState({openInviteButton: true})} color="primary" variant="contained">
                            Invite Admin
                        </Button>
                    ): (
                        <div>
                            ...
                            />
                            <Button style={{marginTop: '10px', width: '30%'}} onClick={this.invite.bind(this)} color="primary" variant="contained">
                                Send
                            </Button>
                        </div>
                    )}
    ...
    invite () {
        if (!/\S+@\S+\.\S+/.test(this.state.email))
            return this.setState({errorEmail: "Invalid email"});
